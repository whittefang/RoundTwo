...
namespace RoundTwoMono
{
    class ChunLiDriver : Component
    {
        ...
        public void InitializeComponents(PlayerIndex index) {
            
            state = new FighterStateHandler(index);
            entity.addComponent(state);

            attackPlayer = new AttackPlayer();
            entity.addComponent(attackPlayer);

            actionManager = new ActionManager();
            entity.addComponent(actionManager);

            hitResolver = new HitResolver();
            entity.addComponent(hitResolver);
            ...
            playerMovement = new PlayerMovement(5f);
            entity.addComponent(playerMovement);

            superMeter = new SuperMeter();
        ...
        public override void Load(ContentManager content)
        {
            ...
            loadInputs(content);
            loadProjectile(content);

            playerMovement.Load(content);
            
            hitResolver.SetThrows(chunLiThrown);
        ...
        public void loadAttacks(ContentManager content)
        {
            ...
            actionFrame = new ActionFrame(3);
            actionFrame.optionalHitFunction = () => { actionManager.StartAttack(jumpMediumFollowup, FighterAnimations.jumpRising); playerMovement.CancelJump(); };
            ...
            actionFrame = new ActionFrame(3);
            actionFrame.optionalFunction = () => { state.SetState(FighterState.invincible); };
            ...
            actionFrame.optionalFunction = () => { state.SetState(FighterState.attackRecovery); };
            sp2.AddActionFrame(actionFrame);
            ...
            actionFrame = new ActionFrame(8);
            actionFrame.optionalFunction = () => { state.SetState(FighterState.projectileInvincible); };
            actionFrame.setMovement(new Vector2(7, -10));
            sp3.AddActionFrame(actionFrame, 8);
            ...
            actionFrame.optionalHitFunction = () => { actionManager.StartAttack(throwAttack, FighterAnimations.throwComplete); };
        ...
        public void loadProjectile(ContentManager content)
        {
        ...
        public void loadInputs(ContentManager content) {
        ...
        public bool LightAttack()
        {
            ...
            {
                actionManager.StartAttack(light, FighterAnimations.light);
                return true;
            } else if (state.GetState() == FighterState.jumping){
                actionManager.StartAttack(jumpLight, FighterAnimations.jumpLight, false);
                return true;
            }
            return false;
        }
        ...
        {
            if (state.GetState() == FighterState.neutral || playerMovement.cancelState == CancelState.light)
            {
                actionManager.StartAttack(medium, FighterAnimations.medium);
                return true;
            }
            else if (state.GetState() == FighterState.jumping || (state.GetState() == FighterState.jumpingAttack && jumpMovesRemaining))
            {
                jumpMovesRemaining = false;
                actionManager.StartAttack(jumpMedium, FighterAnimations.jumpMedium, false);
                return true;
            }
            return false;
        }
        ...
        {
            if (state.GetState() == FighterState.neutral || playerMovement.cancelState == CancelState.medium)
            {
                actionManager.StartAttack(heavy, FighterAnimations.heavy);
                return true;
            }
            else if (state.GetState() == FighterState.jumping)
            {
                actionManager.StartAttack(jumpHeavy, FighterAnimations.jumpHeavy, false);
                return true;
            }
            return false;
        }
        ...
        {
            if (!fireball.isActive && (state.GetState() == FighterState.neutral || playerMovement.cancelState == CancelState.light || playerMovement.cancelState == CancelState.medium || playerMovement.cancelState == CancelState.heavy))
            {
                superMeter.AddMeter(50);
                actionManager.StartAttack(sp1, FighterAnimations.sp1);
                return true;
            }
            return false;
        }
        public void ActivateFireabll() {
            ...
            fireball.Activate(transform.position, state.GetFacingDirection());
            MasterSound.fireball.Play();
        }
        public void FireballHit() {
            if (state.GetState() == FighterState.attackStartup) {
                playerMovement.cancelState = CancelState.special;
            }
        }
        public bool sp2Attack()
        {
            if (state.GetState() == FighterState.neutral || playerMovement.cancelState == CancelState.light || playerMovement.cancelState == CancelState.medium || playerMovement.cancelState == CancelState.heavy)
            {
                superMeter.AddMeter(75);
                actionManager.StartAttack(sp2, FighterAnimations.sp2);
                return true;
            }
            return false;
        }
        public bool sp3Attack()
        {
            ...
            {
                superMeter.AddMeter(75);
                actionManager.StartAttack(sp3, FighterAnimations.sp3);
                return true;
            }
            return false;
        }
        ...
        {
            if ((state.GetState() == FighterState.neutral || playerMovement.cancelState == CancelState.light || playerMovement.cancelState == CancelState.medium || playerMovement.cancelState == CancelState.heavy || playerMovement.cancelState == CancelState.special) && superMeter.GetMeter() >= 1000)
            {
                superMeter.EmptyMeter();
                playerMovement.transform.position.Y = playerMovement.groundBound;
                actionManager.StartAttack(super, FighterAnimations.Super);
                return true;
            }
            return false;
        }
        ...
        public bool ThrowAttackFunc()
        {
            if (state.GetState() == FighterState.neutral )
            {
                actionManager.StartAttack(throwTry, FighterAnimations.throwTry);
                return true;
            }
            return false;
        }
